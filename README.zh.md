...
- 公开 Proof index：静态首页会把 OID Knowledge Lab、Signal Garden、Incident Zero Stack、Hanzi Scout 和 Helioigma 的公开可验证入口集中到一块，方便编辑、评审或技术负责人快速查看。
- 客户就绪包：`reports/client-readiness-pack.md` 和 `reports/client-readiness-pack.json`。
- 垂直场景适配包：`reports/vertical-use-case-pack.md` 和 `reports/vertical-use-case-pack.json`，把同一套审计能力映射到 SNMP/MIB、PKI 证书策略 OID、企业内部 OID registry 清理三类场景。
- 范围建议包：`reports/scope-proposal-pack.md` 和 `reports/scope-proposal-pack.json`，把样例审计整理成第一阶段范围、前 48 小时动作、客户输入、验收标准和排除项。
- 工作说明包：`reports/statement-of-work-pack.md` 和 `reports/statement-of-work-pack.json`，把第一阶段范围进一步整理成目标、时间表、交付物、客户责任、验收清单、变更控制和排除项。
- 决策一页摘要：`reports/decision-one-pager.md` 和 `reports/decision-one-pager.json`，把是否批准小范围审计、下一步输入、交付物、边界和证据链接压缩到一页。
- 客户启动包：`reports/client-kickoff-pack.md` 和 `reports/client-kickoff-pack.json`，把一页摘要转换成初次回复、安全输入请求、首次电话议程、交付物预览、验收预览和证据链接。
- 静态检索面板：`public/index.html`。
- 浏览器版样例交付页：`public/sample-assessment.html`。
- 评审快速阅读指南：`docs/client-review-guide.md`。
...
- 脱敏交付包。
- 客户就绪包：把 intake、浏览器本地审计、样例交付、修复队列、来源边界和验收证据串成一张公开评审地图。
- 垂直场景适配包：把样例审计和公开来源边界映射到 SNMP/MIB 厂商 PEN 盘点、PKI 证书策略 OID 审查、企业内部 OID registry 清理三类可解释场景。
- 范围建议包：把样例审计收束成一个可以批准的小范围第一阶段。
- 工作说明包：把范围建议转换成更接近客户签收的交付边界、客户责任、验收清单和变更控制。
- 决策一页摘要：把客户或评审最先需要判断的问题压缩成一页，包括为什么现在做、下一步谁做、需要什么安全输入、交付什么、哪些数据明确排除。
- 客户启动包：把一页摘要进一步变成客户来问时可以直接使用的材料，包括初次回复、安全 intake、首次电话议程、交付物预览和公开证据链接。
...
7. `docs/client-review-guide.md`：看评审或客户如何快速扫完整个公开证据包。
8. `reports/client-readiness-pack.md`：看客户审计开始前需要什么、会产出什么、如何验收、哪些数据不能进入仓库。
9. `reports/vertical-use-case-pack.md`：看这个项目如何贴近 SNMP/MIB、PKI 和内部 registry 三种真实场景。
10. `reports/scope-proposal-pack.md`：看第一阶段范围、前 48 小时动作、客户输入和验收标准。
11. `reports/decision-one-pager.md`：先看客户/评审第一眼决策摘要。
12. `reports/statement-of-work-pack.md`：看更正式的工作边界、交付物、客户责任和变更控制。
13. `reports/sample-delivery-pack.md`：看未来可以交付给客户的报告形态。
14. `docs/authorized-full-crawl.zh.md`：如果以后拿到授权，再看全量正文采集流程。
